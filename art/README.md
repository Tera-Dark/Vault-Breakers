...
`src/StarterPlayerScripts/UI/VaultPreview.luau` 生成原创的微缩金库、圆头方块选手和演播摄像机，直接放在三个 ViewportFrame 中。它们是菜单插画式模型，不是正在进行的对局快照，不读取奖池映射或克隆其他玩家角色。205 个本地零件只在菜单首次打开时创建；没有把用户提供的参考截图裁成按钮，也不要求上传图片/网格。

品牌和菜单使用 Roblox 自带的 Fredoka One 与系统回退字形，保留项目自身的金库主题。离线 review 可选使用 OFL 授权的 Fredoka One / Noto CJK 参考字体（只下载到忽略目录），不将字体文件混入 place。它们不能证明 Roblox 的实际中文排版。
